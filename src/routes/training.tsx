...
export const Route = createFileRoute("/training")({
  head: () => ({
    meta: [
      { title: "Training Calendar & Session Designer — T4P — Training 4 Performance" },
      {
        name: "description",
        content:
...
function TrainingPage() {
  const [selectedId, setSelectedId] = useState(sessionCalendar.find((s) => s.date === today)!.id);
  ...
  const plan = useMemo(() => {
    ...
    const minutes = list.length ? Math.round(session.durationMin) : 0;
  ...
  return (
    <AppShell title="Training" subtitle="Calendar · participation · session designer">
      <section className="panel p-4">
        <SectionTitle title="Microcycle" hint="Select a day to open the session" />
